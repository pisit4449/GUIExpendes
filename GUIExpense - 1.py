...
#สร้าง database
conn = sqlite3.connect('expense.sqlite3')  # expense ชื่อ database
#สร้างตัวดำเนินการ (อยากได้อะไรใช้ตัวนี้เลย) curcor()
c = conn.cursor()

#สร้าง table ด้วยภาษา sql
''' 
'ID (transactionid) TEXT',
'วัน-เวลา (datetimes)' TEXT,
'รายการ (title)' TEXT,
'ราคา (expense)' REAL(FLOAT),
...
# ฟังชั่นใส่ข้อมูลในตาราง
def insert_expense(transactionid,datetimes,title,expense,quantity,total):
    ID = None
    with conn:
        c.execute("""INSERT INTO expenselist VALUES (?,?,?,?,?,?,?)""",
        (ID,transactionid,datetimes,title,expense,quantity,total))
    conn.commit()  #การบันทึกข้อมูลลงฐานข้อมูลถ้าไม่รันตัวนี้จะไม่บันทึก
...
def Save(event=None):
    ...
    try:
        ...
        ########## Data Base ##########
        insert_expense(transactionid, dt, expense, float(price), pic, float(total))
        ###############################
